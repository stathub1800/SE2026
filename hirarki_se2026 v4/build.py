...
K["kk_didata"]   = kk.kolom("bangunan K1 didata",  ["khusus", "didata"], tanpa=["persentase", "berpindah"])
K["p_kk"]        = kk.kolom("% bangunan K1 didata",["persentase", "khusus", "didata"], tanpa=["berpindah"])
...
for nm, lem, ada, daun in [
    ...
    ("kk_pindah", kk, [], "Jumlah Bangunan Keluarga Khusus Berpindah-pindah Didata (K2)"),
    ...
    ("ak_khusus",     ak,  [], "Anggota Keluarga Khusus"),   # dihapus FASIH sejak 23 Agu 2026
    ("uk_tutup", uk, [], "Tutup"),
    ("uk_ganda", uk, [], "Ganda"),
...
def baca_kondef(path, sheet):
    """Ambil blok konsep-definisi yang ditulis FASIH di bawah tabel."""
    d = pd.read_excel(path, sheet_name=sheet, header=None)
    akhir = max((i for i in range(len(d)) if str(d.iloc[i, 0]).strip().isdigit()), default=0)
    out = {}
    for i in range(akhir + 1, len(d)):
        k = bersih(d.iloc[i, 0])
        v = bersih(" ".join(str(x) for x in d.iloc[i, 1:].tolist() if str(x) != "nan"))
        if k and len(v) > 40:
            out[k.lstrip("*")] = v
    return out

KONDEF = {}
for _sh in ("PROGRES PENDATAAN", "USAHA PERUSAHAAN", "SKALA USAHA", "USAHA KELUARGA", "KESELURUHAN USAHA"):
    KONDEF.update(baca_kondef(F1, _sh))
for _sh in ("KELUARGA", "ANGGOTA KELUARGA", "KELUARGA KHUSUS"):
    KONDEF.update(baca_kondef(F2, _sh))

# kartu -> kunci definisi di berkas
PETA_KONDEF = {
 "assign":   "Prelist",
 "capaian":  "Hasil Verifikasi",
 "responden":"Responden Didata (Usaha & Keluarga Ditemukan/Baru/Force Submit)",
 "draft":    "Jumlah Responden Sedang Didata",
 "open":     "Jumlah Prelist Belum Didata",
 "ub":       "Jumlah Usaha (UB/UM/UMK)",
 "um":       "Jumlah Usaha (UB/UM/UMK)",
 "umk":      "Jumlah Usaha (UB/UM/UMK)",
 "unk":      "Tidak Dapat Diklasifikasikan (Kolom 10)",
 "usaha":    "Jumlah Usaha BKU",
}
...
sel = [(rows[k]["nama"], rows[k]["verif"]+rows[k]["draft"]+rows[k]["open"]-rows[k]["tot_assign"])
       for k in kodes_semua]
tak_pas = [f"{a} ({b:+,})".replace(",", ".") for a, b in sel if b]
if tak_pas:
    print("  CATATAN verifikasi + draft + open tidak persis sama dengan total assignment di "
          f"{len(tak_pas)} wilayah. Provinsi: {sel[0][1]:+,}.".replace(",", "."))
    print("          FASIH sendiri mencatat Draft dan Open dapat berbeda dari hasil pengurangan")
    print("          indikator lain karena perbedaan waktu pemrosesan data. Bukan kesalahan tarikan.")
...
N=[
 ...
 dict(id='unk',parent='usaha',label='Unit Pembantu',sub='Penunjang, pendapatan nol',field='unk',base='usaha_unit',unit='usaha',tipe='bagi',warna='abu',
   ket='Usaha baru dengan nilai omzet atau pendapatan kosong sehingga skala usahanya tidak dapat diklasifikasikan, misalnya usaha unit pembantu atau penunjang. Ini kondisi wajar, bukan isian yang kurang lengkap. FASIH menggabungkannya dengan UMK dan menuliskannya sebagai UMK*.',
 ...
 dict(id='kkhusus',parent='keluarga',label='Keluarga Khusus',sub='Bangunan menetap (K1)',field='kk_didata',base=None,unit='bangunan',tipe='tautan',warna='merah',
   ket='Keluarga yang tinggal di barak, pondok pesantren, hunian sementara, pengungsian, dan sejenisnya. Didata terpisah lewat dokumen K1 untuk bangunan menetap, sehingga satuan yang dilaporkan adalah bangunan, bukan keluarga. Sejak ekspor 23 Agustus 2026 FASIH memisahkan bangunan berpindah-pindah ke dokumen K2; angkanya ada pada kolom tambahan di tabel bawah.',
   rumus='bangunan menetap (K1) didata \u00f7 bangunan hasil pendataan PPL',tautan_ket='Jalur K1, satuan bangunan',
...
EKSTRA = {
 ...
 "umk":      [("Unit pembantu","unk"),("UMK* (UMK + unit pembantu)","umk_bintang"),
              ("Door-to-door ditemukan","mk_ok"),("Door-to-door baru","mk_baru"),
              ("Door-to-door tutup","mk_tutup"),("Door-to-door ganda","mk_ganda"),
              ("Door-to-door tidak ditemukan","mk_tt")],
 "unk":      [("UMK","umk"),("UMK* (UMK + unit pembantu)","umk_bintang")],
 "force":    [("Usaha BKU","bku"),("Door-to-door ditemukan","mk_ok"),("Door-to-door baru","mk_baru")],
 ...
 "ak":       [("Meninggal","ak_meninggal"),("Pindah dalam negeri","ak_dn"),("Pindah luar negeri","ak_ln"),
              ("Tidak ditemukan","ak_tt")],
 "kkhusus":  [("Berpindah-pindah didata (K2)","kk_pindah"),("Hasil listing PPL","kk_bangunan")],
}
for n in N:
...
for n in N:
    k = PETA_KONDEF.get(n["id"])
    if k and k in KONDEF:
        n["kondef"] = KONDEF[k]
KURANG = [i for i, k in PETA_KONDEF.items() if k not in KONDEF]
if KURANG:
    print("  CATATAN definisi resmi tidak ditemukan di berkas untuk kartu: " + ", ".join(KURANG))
...
# ================================================================ Radar Anomali & Monitoring SLS
# Nama anomali usaha di bawah diambil dari daftar anomali yang disusun Pokja TI.
# COCOKKAN URUTANNYA dengan radar FASIH; ubah di sini bila berbeda.
# Anomali keluarga belum bernama - isi sendiri bila kondefnya sudah ada.
NAMA_ANOMALI = {
 "usaha": {
   1: ("Bukan produksi sendiri, biaya produksi besar",
       "Usaha menyatakan barang bukan produksi sendiri, tetapi biaya produksi melebihi separuh total pengeluaran."),
   2: ("Pengeluaran melebihi pendapatan",
       "Total pendapatan dikurangi total pengeluaran bernilai negatif."),
   3: ("Badan usaha tidak sesuai penyertaan modal",
       "Bentuk badan usaha tidak berbadan hukum, tetapi ada penyertaan modal publik atau non-publik."),
   4: ("Rasio pendapatan MBG janggal",
       "Usaha berperan dalam program MBG dengan rasio pendapatan terhadap pengeluaran di luar rentang wajar."),
   5: ("Aset sangat besar, skala usaha kecil",
       "Aset di atas 10 miliar dengan satu tenaga kerja dan pendapatan di bawah 60 juta."),
   6: ("Tanpa internet, pendapatan sangat besar",
       "Tidak menggunakan internet namun pendapatan mencapai 15 miliar atau lebih."),
   7: ("Tanpa laporan keuangan, pendapatan sangat besar",
       "Tidak menyusun laporan keuangan namun pendapatan mencapai 15 miliar atau lebih."),
   8: ("KBLI akhir beda kategori dari prelist",
       "Dua digit awal KBLI hasil pencacahan berbeda dari KBLI prelist."),
 },
 "keluarga": {},
}

def baca_radar(pola, jenis):
    hit = sorted(DATA.glob(pola))
    if not hit:
        return None
    path = max(hit, key=lambda p: p.stat().st_mtime)
    d = pd.read_excel(path, sheet_name=0, header=None)
    kol = [bersih(x) for x in d.iloc[3].tolist()]
    mulai = next(i for i in range(len(d)) if str(d.iloc[i, 0]).strip().isdigit())
    d = d.iloc[mulai:].copy()
    d = d[pd.to_numeric(d[0], errors="coerce").notna()]
    d[0] = d[0].astype(int).astype(str)
    d = d.set_index(0)
    nomor = sorted({int(re.search(r"Anomali (\d+)", k).group(1))
                    for k in kol if re.match(r"^Anomali \d+ - Belum", k)})
    def idx(n, apa):
        for i, k in enumerate(kol):
            if k.startswith(f"Anomali {n} - {apa}"):
                return i
        sys.exit(f"GAGAL: kolom 'Anomali {n} - {apa}' tidak ada di {path.name}")
    ikut = lambda k, c: 0 if pd.isna(d.loc[k, c]) else int(d.loc[k, c])
    daftar = []
    for n in nomor:
        cb, cs, cx = idx(n, "Belum"), idx(n, "Sudah"), idx(n, "Sesuai")
        nama, ket = NAMA_ANOMALI[jenis].get(n, ("", ""))
        daftar.append({"no": n, "nama": nama, "ket": ket,
                       "data": {k: [ikut(k, cb), ikut(k, cs), ikut(k, cx)] for k in d.index}})
    tot = {k: ikut(k, 2) for k in d.index}
    return {"item": daftar, "tot": tot, "berkas": path.name, "jam": jam(path)}
...
def baca_sls(pola):
    ...
    d[0] = d[0].astype(int).astype(str)
    d = d.set_index(0)
    return {"data": {k: [int(d.loc[k, 2]), int(d.loc[k, 3])] for k in d.index},
            "berkas": path.name, "jam": jam(path)}


AN_U = baca_radar("*Radar*Anomali*Usaha*.xlsx", "usaha")
AN_K = baca_radar("*Radar*Anomali*Keluarga*.xlsx", "keluarga")
SLS  = baca_sls("*Monitoring*SLS*.xlsx")

print("\nBerkas tambahan:")
for lb, o in (("Radar Anomali Usaha", AN_U), ("Radar Anomali Keluarga", AN_K), ("Monitoring SLS", SLS)):
    print(f"  {lb:<24}" + (f"{o['berkas'][:52]}  ({o['jam']})" if o else "tidak ada, tab disembunyikan"))

for lb, o in (("Anomali usaha", AN_U), ("Anomali keluarga", AN_K)):
    if not o:
        continue
    salah = [k for k in o["tot"] if sum(sum(it["data"][k]) for it in o["item"]) != o["tot"][k]]
    print(f"  {'OK    ' if not salah else 'GAGAL '}{lb}: jumlah seluruh anomali = Total Assignment"
          + ("" if not salah else "  -> " + ", ".join(salah)))
    tanpa_nama = [str(it["no"]) for it in o["item"] if not it["nama"]]
    if tanpa_nama:
        print(f"        CATATAN anomali belum bernama: {', '.join(tanpa_nama)} "
              f"(isi di NAMA_ANOMALI dalam build.py)")
if SLS:
    p_ = SLS["data"]["18"]
    print(f"  OK    SLS: selesai {p_[1]:,} dari target {p_[0]:,} ({p_[1]/p_[0]*100:.2f}%), "
          f"sisa {p_[0]-p_[1]:,}".replace(",", "."))
...
muatan = json.dumps({"rows": rows, "kodes": kodes, "nodes": N, "stempel": stempel,
                     ...
                     "anomali": {"usaha": AN_U, "keluarga": AN_K}, "sls": SLS},
